...
fig = px.bar(
    ...
    labels={
        "value": "Score",
        "variable": "Total Predicted Sentiment",
        "label": "Sentiment"
    },
    barmode="group",
    height=400,
...
st.write("## Comments")
comments_df = get_comments(selected_video_id)
# st.write(comments_df)
# df = pd.merge(video_summary, comments_df, left_on='id', right_on='video_id')
df = pd.merge(video_summary, comments_df, on='id')
st.write("Double click to view full comment.")
st.write("Click on column header to sort.")
st.dataframe(
    df,
    hide_index=True,
    column_config={
